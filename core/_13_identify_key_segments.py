...
def identify_key_segments_from_subtitles(
    ...
    # 1. 如果没有传入 df_trans_time，读取 `_6_gen_sub.align_timestamp_main()` 写入的 `trans.srt`
    #    align_timestamp_main() 会在 `_OUTPUT_DIR` 下生成 'trans.srt'，每一项为最终展示的中文字幕。
    if df_trans_time is None:
        srt_path = os.path.join(_OUTPUT_DIR, 'trans.srt')
        if os.path.exists(srt_path):
            # 解析 SRT 文件为 DataFrame，保留 timestamp 和 Translation 列，格式与 align_timestamp() 输出一致
            entries = []
            with open(srt_path, 'r', encoding='utf-8') as f:
                content = f.read().strip()
            if content == '':
                console.print(f"[red]❌ Error: {srt_path} is empty[/red]")
                raise FileNotFoundError(f"Empty subtitle file: {srt_path}")
            ...
            for block in raw_blocks:
                ...
                if len(lines) >= 2:
                    ...
                    if re.match(r'^\d+$', lines[0]):
                        ...
                        # no numeric index, assume first line is timestamp
                        timestamp = lines[0]
                        text = ' '.join(lines[1:])

                    entries.append({'timestamp': timestamp, 'Translation': text})

            if not entries:
                console.print(f"[red]❌ Error: no subtitle entries parsed from {srt_path}[/red]")
                raise ValueError(f"No subtitle entries in {srt_path}")

            df_trans_time = pd.DataFrame(entries)
            console.print(f"[cyan]Using subtitle file: {srt_path} ({len(df_trans_time)} lines)[/cyan]")
        else:
            console.print("[red]❌ Error: expected subtitle file not found: output/trans.srt[/red]")
            console.print("[yellow]Please run `_6_gen_sub.align_timestamp_main()` to generate subtitles (trans.srt) in the `output` folder[/yellow]")
            raise FileNotFoundError("output/trans.srt not found")
    ...
    # 3. 计算视频总时长
    # 尝试从最后一行的 timestamp 提取结束时间（兼容多种格式）
    if 'timestamp' not in df_trans_time.columns:
        # 回退：如果存在 numeric 的 end_time 列，取最大值
        if 'end_time' in df_trans_time.columns:
            video_total_seconds = float(df_trans_time['end_time'].max())
        else:
            raise ValueError("timestamp column not found in df_trans_time")
    else:
        last_timestamp = str(df_trans_time.iloc[-1]['timestamp'])
        # 常见格式: "HH:MM:SS,mmm --> HH:MM:SS,mmm"
        if ' --> ' in last_timestamp:
            end_time_str = last_timestamp.split(' --> ')[1].strip()
            video_total_seconds = parse_srt_time(end_time_str)
        else:
            # 可能只有单一时间或已经是结束时间
            end_time_str = last_timestamp.strip()
            # 如果是纯数字（秒），直接转换
            if re.match(r'^\d+(?:\.\d+)?$', end_time_str):
                video_total_seconds = float(end_time_str)
            else:
                # 退回到 numeric end_time 列
                if 'end_time' in df_trans_time.columns:
                    video_total_seconds = float(df_trans_time['end_time'].max())
                else:
                    # 最后尝试用 parse_srt_time 解析单个时间字符串
                    video_total_seconds = parse_srt_time(end_time_str)
