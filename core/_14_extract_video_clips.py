    ...
    with Progress(
        ...
        for segment in key_segments:
            ...
            try:
                ...
                # Check if we should burn subtitles into the clip. Prefer final translated subtitles
                subtitle_file = os.path.join(_OUTPUT_DIR, 'trans.srt')
                burn_subs = os.path.exists(subtitle_file) and load_key('burn_subtitles')

                if use_copy_codec and not burn_subs:
                    # 快速模式：直接复制编码
                    cmd = [
                        'ffmpeg',
                        '-i', video_file,
                    ...
                    # 需要烧录字幕或用户要求转码，使用 libx264 并添加字幕滤镜（如果存在）
                    cmd = [
                        'ffmpeg',
                        '-i', video_file,
...
def create_dubbed_summary_from_segments(
    key_segments: List[Dict],
    merged_video: str = "output/batch_summary/merged_clips.mp4",
    output_file: str = "output/batch_summary/merged_clips_dub.mp4",
    normalized_dub_audio: str = "output/normalized_dub.wav"
):
    """
    Create a dubbed version of the merged summary by slicing the full normalized dubbed audio
    according to key segment timestamps, concatenating the slices, and remuxing into the
    merged video. Falls back to 'output/dub.mp3' if normalized wav is not available.
    """
    console.print(Panel(f"[bold cyan]🔊 Creating dubbed summary audio[/bold cyan]"))

    # Determine source dubbed audio
    src_audio = normalized_dub_audio if os.path.exists(normalized_dub_audio) else os.path.join(_OUTPUT_DIR, 'dub.mp3')
    if not os.path.exists(src_audio):
        console.print(f"[red]❌ No source dubbed audio found: {src_audio}[/red]")
        raise FileNotFoundError(f"No dubbed audio: {src_audio}")

    tmp_dir = os.path.join(os.path.dirname(merged_video), "_tmp_audio_segments")
    ...
    try:
        for i, seg in enumerate(key_segments):
            ...
            out_seg = os.path.join(tmp_dir, f"seg_{i:02d}.wav")
            # Extract segment from dubbed audio and re-encode to wav (stable)
            cmd = [
                'ffmpeg', '-y', '-ss', str(start), '-to', str(end), '-i', src_audio,
                '-ar', '16000', '-ac', '1', out_seg
            ]
        ...
        if not seg_files:
            console.print(f"[red]❌ No audio segments extracted[/red]")
            raise RuntimeError("No audio segments extracted for dubbing")
        ...
        merged_audio = os.path.join(tmp_dir, "merged_summary_audio.wav")
        cmd_concat = [
        ...
        # Remux merged audio into the merged video
        os.makedirs(os.path.dirname(output_file), exist_ok=True)
        cmd_remux = [
        ...
        if os.path.exists(output_file):
            console.print(f"[bold green]✅ Dubbed summary created: {output_file}[/bold green]")
            return output_file
        else:
            raise RuntimeError("Failed to create dubbed summary output file")
